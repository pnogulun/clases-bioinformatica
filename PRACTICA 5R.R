...
plot(ko.mean - wt.mean,xlab="WT", ylab = "KO", main = "GSR5583-ScatterPlot",col="blue",xlim=c(0,limit), ylim=c(0,limit))

# Añadir una línea diagonal con abline
abline(0,1,col="red")# y = x

# y = bx + a
# albine (a,b)
abline(1,2,col="orange") # y = 2x + 1
abline(h=2,col="green") # y = 2
...
plot(diff.mean,-log10(pvalue), main="GS5583 - Volcano")
...
abline(v=diff.mean_cutoff,col="blue",1wd=3)
abline(h=-log10(pvalue_cutoff),col="green",1wd=3)

# Ahora buscamos los genes que satisfagan estos criterios
# Primero hacemos el filtro para la diferencia de medias (fold)
head(data)

filter_by_mean = abs(diff.mean >= diff.mean_ cutoff
dim(data[filter_by_mean,])
...
filtered_combined = filter_by_mean & filter_by_pvalue
